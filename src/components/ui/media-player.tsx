...
/* eslint-disable @next/next/no-img-element, @typescript-eslint/no-empty-object-type, react-hooks/exhaustive-deps, react-hooks/immutability, react-hooks/preserve-manual-memoization, react-hooks/purity, react-hooks/refs, react-hooks/set-state-in-effect */
...
function MediaPlayerSeek(props: MediaPlayerSeekProps) {
  ...
  const seekableRange = Math.max(0, seekableEnd - seekableStart);
  ...
  const onHoverProgressUpdate = React.useCallback(() => {
    if (!seekRef.current || seekableRange <= 0) return;

    const hoverPercent = Math.min(
      100,
      ((hoverTimeRef.current - seekableStart) / seekableRange) * 100,
    );
    seekRef.current.style.setProperty(
      SEEK_HOVER_PERCENT,
      `${hoverPercent.toFixed(4)}%`,
    );
  }, [seekableRange, seekableStart]);
  ...
  const bufferedProgress = React.useMemo(() => {
    if (mediaBuffered.length === 0 || seekableRange <= 0) return 0;
    ...
    if (containingRange) {
      return Math.min(1, (containingRange[1] - seekableStart) / seekableRange);
    }
  ...
  const getSeekTimeFromPointer = React.useCallback(
    (clientX: number) => {
      ...
      if (!seekRect || seekableRange <= 0) return null;
      ...
      return seekableStart + relativeX * seekableRange;
    },
    [seekableRange, seekableStart],
  );
  ...
  }, [seekableRange, onTooltipPositionUpdate, tooltipDisabled]);
  ...
  const onPointerMove = React.useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      rafIdRef.current = requestAnimationFrame(() => {
        ...
        const calculatedHoverTime = getSeekTimeFromPointer(clientX);
        const seekRect = seekRectRef.current;
        if (!seekRect || calculatedHoverTime === null) {
          rafIdRef.current = null;
          return;
        }
  ...
  const onPointerDown = React.useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      if (event.button !== 0 || isDisabled || seekableRange <= 0) return;
  ...
  const SeekSlider = (
    <div data-slot="media-player-seek-container" className="relative w-full">
      <SliderPrimitive.Root
        ...
        step={0.01}
        className={cn(
          "group relative flex w-full touch-none select-none items-center data-disabled:pointer-events-none data-disabled:opacity-50",
          className,
        )}
        value={[displayValue]}
      ...
      >
        <SliderPrimitive.Control className="relative flex w-full items-center">
          <SliderPrimitive.Track className="relative h-1.5 w-full grow overflow-hidden rounded-full bg-white/20 transition-all delay-50 duration-500 group-hover:h-2.5">
            <div
            ...
            />
            <SliderPrimitive.Indicator className="absolute h-full bg-white will-change-[width]" />
          ...
          <SliderPrimitive.Thumb className="relative z-10 block size-2.5 shrink-0 rounded-full bg-white opacity-0 shadow-sm ring-white/30 transition-[color,box-shadow,opacity] will-change-transform group-hover:opacity-100 hover:ring-4 focus-visible:outline-hidden focus-visible:ring-4 disabled:pointer-events-none disabled:opacity-50" />
