...
function decode(value: string) {
  return JSON.parse(gunzipSync(Buffer.from(value, "base64")).toString("utf8")) as EpgWindow
}
...
function stripDescriptions(window: EpgWindow): EpgWindow {
  ...
  for (const [channelId, slots] of Object.entries(window.channels)) {
    channels[channelId] = slots.map(([start, stop, title]) => [start, stop, title])
  }
...
function splitIntoChunks(window: EpgWindow) {
  ...
  for (const [channelId, slots] of Object.entries(window.channels)) {
    for (const slot of slots) {
      ...
      const final = Math.min(finalChunk, alignChunkStart(Math.max(slot[0], slot[1] - 1)))
...
async function publishCountry(country: string, window: EpgWindow) {
  ...
  const bytes = encoded.reduce((total, item) => total + Buffer.byteLength(item.value), 0)
  if (bytes > MAX_SOURCE_BYTES) throw new Error(`${country} guide exceeds the 32 MB cache limit.`)
  ...
  return Boolean(await withPublishLock(async () => {
    ...
    if (total > MAX_TOTAL_BYTES) throw new Error("IPTV-EPG cache budget is full.")

    const version = `${Date.now()}-${crypto.randomUUID().slice(0, 8)}`
    const chunkKeys = encoded.map(({ chunk }) => chunkKey(country, version, chunk.from))
    const pipeline = redis.pipeline()
    for (let index = 0; index < encoded.length; index += 1) {
      pipeline.set(chunkKeys[index]!, encoded[index]!.value, { ex: CACHE_TTL_SECONDS })
    }
    await pipeline.exec()

    const manifest: CachedManifest = { version, updatedAt: Date.now(), bytes, chunkKeys }
...
export async function requestIptvEpgRefresh(country: string) {
  ...
  return Boolean(await redis.set(refreshRequestedKey(code), "1", { nx: true, ex: 5 * 60 }))
}
...
export async function getCachedIptvEpgWindow(
  ...
  const starts: number[] = []
  for (let start = alignChunkStart(from); start < to; start += CHUNK_MS) starts.push(start)
  const values = await redis.mget<string[]>(...starts.map((start) => chunkKey(code, manifest.version, start)))
  ...
  for (const value of values) {
    ...
    for (const [channelId, slots] of Object.entries(chunk.channels)) {
      ...
      for (const slot of slots) {
        if (slot[1] > from && slot[0] < to && !target.some((existing) => existing[0] === slot[0] && existing[1] === slot[1] && existing[2] === slot[2])) target.push(slot)
      }
    }
  }
  for (const slots of Object.values(channels)) slots.sort((left, right) => left[0] - right[0])
...
export async function refreshIptvEpgCountry(country: string) {
  ...
  const window = await fetchEpgWindow(source.url, { from, hours: HOT_WINDOW_MS / (60 * 60 * 1000), includeDescriptions: true })
...
export async function getActiveIptvEpgCountries() {
  ...
  const active = members.length
    ...
    : []
  return members.filter((country, index) => active[index] && Boolean(getIptvEpgSource(country)))
}
